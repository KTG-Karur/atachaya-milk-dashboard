...
const CancelReport = ({ navigation }) => {
    ...
    return (
        <>
            {
                viewModule && <TempView title={`Cancel Quotations Details`} closeModule={() => onViewForm()}>
                    <InvoiceFiveprint data={state?.quotationDetailsList || []} />
                </TempView>
            }
            {
                parentViewModule &&   
                <TemplateCustomTable columns={columns} list={parentList}  state={state} setState={setState} reportCol={col} reportData={state?.quotationReportList || []} reportCount={true} countName={"Cancel Count"} count={state?.count || 0} tableName={"Cancel Quotation Report"} show={show} setShow={setShow} totalCount={state?.totalCount || 0} />
            }
        </>
    );
}
